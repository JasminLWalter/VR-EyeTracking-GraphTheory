...
# full data frame
dataP2B <- read.csv("E:/WestbrookProject/SpaRe_Data/control_data/Analysis/P2B_controls_analysis/overviewTable_P2B_Prep_complete.csv")
...
savepath <- "E:/WestbrookProject/SpaRe_Data/control_data/Analysis/P2B_controls_analysis/performanceAnalysis_trialSequence"
...
dataGraphMeasures <- read.csv("F:/WestbrookProject/Spa_Re/control_group/analysis_velocityBased_2023/P2B_analysis/data_overviews/overviewGraphMeasures.csv")
...
#dataGraphM <- cbind(dataGraphMeasures[, c(2,3,4,5,6,7,8,9)])
...
# modelGraph <- lm(meanPerformance ~ nrViewedHouses + density + diameter + 
#                       hierarchyIndex ,data = dataGraphMFRS)

modelGraph <- lm(meanPerformance ~ nrViewedHouses + nrEdges + density + diameter + avgShortestPath,data = dataGraphMFRS)
summary(modelGraph)

# general graph properties and FRS

modelDensity <- lm(meanPerformance ~ density, data = dataGraphMFRS)
summary(modelDensity)

modelNrEdges <- lm(meanPerformance ~ nrEdges, data = dataGraphMFRS)
summary(modelNrEdges)


modelNrNodes <- lm(meanPerformance ~ nrViewedHouses, data = dataGraphMFRS)
summary(modelNrNodes)
...
modelAvgPath <- lm(meanPerformance ~ avgShortestPath, data = dataGraphMFRS)
summary(modelAvgPath)

########
# test sig in one

modelTest <- lm(meanPerformance ~ nrEdges + density + avgShortestPath, data = dataGraphMFRS)
summary(modelTest)

########

modelMeanND <- lm(meanPerformance ~ meanND, data = dataGraphMFRS)
summary(modelMeanND)
...
modelAllLandmarks <- lm(meanPerformance ~ nrAllLandmarks, data = dataGraphMFRS)
summary(modelAllLandmarks)



# general graph properties and FRS + edges!!!!
modelGraphFRS <- lm(meanPerformance ~ Mean_egocentric_global + Mean_survey +
                       Mean_cardinal  ,data = dataGraphMFRS)
summary(modelGraphFRS)


modelEG <- lm(meanPerformance ~ Mean_egocentric_global ,data = dataGraphMFRS)
summary(modelEG)

modelS <- lm(meanPerformance ~ Mean_survey ,data = dataGraphMFRS)
summary(modelS)

modelC <- lm(meanPerformance ~ Mean_cardinal ,data = dataGraphMFRS)
summary(modelC)

modelnrG <- lm(meanPerformance ~ nrGazes ,data = dataGraphMFRS)
summary(modelnrG)
