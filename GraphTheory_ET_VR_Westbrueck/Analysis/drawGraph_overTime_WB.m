...
savepath = 'E:\WestbrookProject\SpaRe_Data\control_data\Analysis\visualization_graph_plots\graphs_over_time\part1013\';
...
scope = [102,240,541,1640];
plotWalkingPath = false;
% load map
...
%         figure(4)
        ...
        for index = [scope,length(cleanData)] 
            ...
            if(index ==1)
                ...
                if(plotWalkingPath)
                    ...
                    % plot location
                    plot([cleanData(index).playerBodyPosition_x]*4.2+2050, [cleanData(index).playerBodyPosition_z]*4.2+2050, 'LineWidth',0.5,'color','red');
                end
            else
                
                if(plotWalkingPath)
                    % plot location
                    plot([cleanData(1:index).playerBodyPosition_x]*4.2+2050, [cleanData(1:index).playerBodyPosition_z]*4.2+2050, 'LineWidth',0.5,'color','red');
                end
    ...
    %                         saveas(gcf, strcat(savepath, num2str(currentPart),'_gif_graph_creation_', num2str(index),'.jpg'));
           ax = gca;
           exportgraphics(ax,strcat(savepath, num2str(currentPart),'_graph_creation_', num2str(index),'.png'),'Resolution',600)
           hold off 
