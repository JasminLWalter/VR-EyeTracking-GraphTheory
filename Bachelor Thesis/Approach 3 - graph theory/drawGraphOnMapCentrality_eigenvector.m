%%-------------------- draw graph on map centrality_eigenvector------------------------
...
savepath = 'D:\BA Backup\Data_after_Script\approach3-graphs\centrality\onMapPlots\eigenvector\';
...
for ii = 1:Number
    ...
    elseif exist(file)==2
        ...
        title({strcat('Graph - eigenvector centrality - Participant- ',num2str(currentPart),' - on map'),'    '});
 ...
 %---------comment code until here to only show nodes without edges--------
      ...
      saveas(gcf,strcat(savepath,'Graph_centralityEigenvector_on_Map_Participant_',num2str(currentPart),'.png'),'png');
