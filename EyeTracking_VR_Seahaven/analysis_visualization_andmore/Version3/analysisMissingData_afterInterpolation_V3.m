%% -----------------analyseMissingData_afterInterpolation----------------------------
...
for ii = 1:Number
    ...
    elseif exist(file)==2
        ...
        for index = 1:length(interpolatedColliders)
            if strcmp(interpolatedColliders(index).Collider,'noData')
                if (index == 1 || index == length(interpolatedColliders))
                    sampleInfo = [sampleInfo; interpolatedColliders(index).Samples];
                    sameDiffInfo = [sameDiffInfo; 'diff'];
                    
                else
                    sampleInfo = [sampleInfo; interpolatedColliders(index).Samples];
                    
                    col1 = interpolatedColliders(index-1).Collider;
                    col2 = interpolatedColliders(index+1).Collider;
                    if(strcmp(col1,col2))
...
save(strcat(savepath, 'missingDataOverview_afterInterpolation'), 'missingDataOverview');
