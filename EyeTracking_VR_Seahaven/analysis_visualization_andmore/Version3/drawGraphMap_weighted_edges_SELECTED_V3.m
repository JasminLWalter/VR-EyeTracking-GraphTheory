...
savepath = 'E:\NBP\SeahavenEyeTrackingData\90minVR\Version03\analysis\weightedGraphs\graphPlots_individual\selection\';
...
for ii = 1:Number
    ...
    elseif exist(file)==2
        ...
        %% select subset of edges to plot
        avgW = mean(weights);
        stdW = std(weights);
        select = avgW + stdW;
        
        selection = weights > select;
        
        weightsS = weights(selection);
        edgeCellS = edgeCell(selection,:);
        ...
        for ee = 1:length(edgeCellS)
            [Xhouse,Xindex] = ismember(edgeCellS(ee,1),coordinateList.House);
            
            [Yhouse,Yindex] = ismember(edgeCellS(ee,2),coordinateList.House);
