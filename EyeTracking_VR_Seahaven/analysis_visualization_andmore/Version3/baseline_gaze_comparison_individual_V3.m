%% ------------------baseline_gaze_comparison_individual_V3----------------------------------------
...
savepath = 'D:\Studium\NBP\Seahaven\90min_Data\Desync_Analysis\baseline_gaze_comparison\';
...
for ii = 1:Number
    ...
    overviewComparison = table2struct(houseTable);
    ...
    for shiftIndex = 1:maxShiftNr
        ...
        elseif exist(file)==2
            ...
            for houseIndex2 = 1:nrHouses
                ...
                % create list of all gazes on house (for each session)
                ...
                % check in which session there are real gazes and set
                % exception for first gaze to save into overview
                firstS = size(allHouseGazes1_R,2)== 0;
                secondS = size(allHouseGazes2_R,2)== 0;
                thirdS = size(allHouseGazes3_R,2)== 0;
                    ...
                    for gazeIndex = 1 : length(allHouseGazes1_B)
                        ...
                        for gazeIndex_R = 1: length(allHouseGazes1_R)
                            ...
                            if(currentSP > lowBoarder && currentSP < highBoarder)
                                ...
                                if(size(fieldnames(overviewComparison),1) == 1)
                                    overviewComparison(houseIndex2).StartPoints = normalisedStartPoint;
                                    overviewComparison(houseIndex2).GazeLengthSamples = allHouseGazes1_R(gazeIndex_R).Samples;

                                else
                                    overviewComparison(houseIndex2).StartPoints = [overviewComparison(houseIndex2).StartPoints, normalisedStartPoint];
                                    overviewComparison(houseIndex2).GazeLengthSamples = [overviewComparison(houseIndex2).GazeLengthSamples, allHouseGazes1_R(gazeIndex_R).Samples];
                    ...
                    for gazeIndex = 1 : length(allHouseGazes2_B)
                        ...
                        for gazeIndex_R = 1: length(allHouseGazes2_R)
                            ...
                            if(currentSP > lowBoarder && currentSP < highBoarder)
                                ...
                                if(size(fieldnames(overviewComparison),1) == 1)
                                    overviewComparison(houseIndex2).StartPoints = normalisedStartPoint;
                                    overviewComparison(houseIndex2).GazeLengthSamples = allHouseGazes2_R(gazeIndex_R).Samples;

                                else
                                    overviewComparison(houseIndex2).StartPoints = [overviewComparison(houseIndex2).StartPoints, normalisedStartPoint];
                                    overviewComparison(houseIndex2).GazeLengthSamples = [overviewComparison(houseIndex2).GazeLengthSamples, allHouseGazes2_R(gazeIndex_R).Samples];
                                end
                             end
                    ...
                    for gazeIndex = 1 : length(allHouseGazes3_B)
                        ...
                        for gazeIndex_R = 1: length(allHouseGazes3_R)
                            ...
                            if(currentSP > lowBoarder && currentSP < highBoarder)
                                ...
                                if(size(fieldnames(overviewComparison),1) == 1)
                                    overviewComparison(houseIndex2).StartPoints = normalisedStartPoint;
                                    overviewComparison(houseIndex2).GazeLengthSamples = allHouseGazes3_R(gazeIndex_R).Samples;
                                else
                                    overviewComparison(houseIndex2).StartPoints = [overviewComparison(houseIndex2).StartPoints, normalisedStartPoint];
                                    overviewComparison(houseIndex2).GazeLengthSamples = [overviewComparison(houseIndex2).GazeLengthSamples, allHouseGazes3_R(gazeIndex_R).Samples];
    ...
    save([savepath num2str(currentPart),'_overview_comparison_baseline_gazes_V3.mat'],'overviewComparison');
