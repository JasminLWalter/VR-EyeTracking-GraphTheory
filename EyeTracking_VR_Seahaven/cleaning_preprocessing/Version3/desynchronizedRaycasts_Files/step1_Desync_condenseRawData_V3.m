...
% overviewAnalysis = array2table(zeros(Number*maxShiftNr,5));
% overviewAnalysis.Properties.VariableNames = {'Participant','Shift','noData_Rows','total_Rows','percentage'};
overviewAnalysis = load(strcat('D:\Studium\NBP\Seahaven\90min_Data\Desynchronization\Desync_condensedColliders\', 'OverviewAnalysis.mat'));
...
for ii = 1:Number
    ...
    for shiftIndex = 1:maxShiftNr
        ...
        file = strcat('desyncRaycast_VP',num2str(currentPart),'_',num2str(currentShift),'sec_shift.3.0.txt');
...
%             % calculate amount of noData rows
    ...
    %                 
                ...
                save([savepath num2str(currentPart) '_' num2str(currentShift) 'sec_shift_condensedColliders_V3.mat'],'condensedData');

                % update overview

                overviewAnalysis.Participant(ii*shiftIndex)= currentPart;
                overviewAnalysis.Shift(ii*shiftIndex) = currentShift;
                overviewAnalysis.noData_Rows(ii*shiftIndex)= NRnoDataRows;
                overviewAnalysis.total_Rows(ii*shiftIndex)= totalRows;

                percent = (NRnoDataRows*100)/totalRows;

                overviewAnalysis.percentage(ii*shiftIndex) = percent;
